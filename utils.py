...
class util:
  ...
  def evaluate_coherence(self, explanation: str) -> Dict:
        """Evaluates the coherence between paragraphs of a given text."""
        # Split by multiple newlines and strip whitespace
        potential_paragraphs = [p.strip() for p in re.split(r'\n{2,}', explanation) if p.strip()]

        # Filter out paragraphs that are likely too short (e.g., less than 10 words)
        ...
        avg_score = round(np.mean([s["score"] for s in coherence_scores]), 4) if coherence_scores else 0.0
  ...
  def dale_chall(self , text) :
      ...
      # Returning the score
      return raw_score
  #The gunning fog formula helps us to measure the clearity and the simplicity of the text .A Gunning Fog Index of 10 or less is considered to be easy to read. A Gunning Fog Index of 11 to 15 is considered to be fairly easy to read. A Gunning Fog Index of 16 to 20 is considered to be difficult to read. A Gunning Fog Index of 21 or more is considered to be very difficult to read.
  #A Gunning Fog Index of 10 or less is considered to be easy to read. A Gunning Fog Index of 11 to 15 is considered to be fairly easy to read. A Gunning Fog Index of 16 to 20 is considered to be difficult to read. A Gunning Fog Index of 21 or more is considered to be very difficult to read.
